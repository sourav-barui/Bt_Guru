...
<!-- Quick Stats Row -->
@php
    $activeCount = $enrollments->where('enrollment_status', 'active')->count();
    $pendingCount = $enrollments->where('enrollment_status', 'pending')->count();
    $paidCount = $enrollments->filter(fn($e) => $e->isPaymentCompleted())->count();
...
<div class="px-4 pb-6">
    ...
    @forelse($enrollments as $enrollment)
        @php 
            ...
            $statusColor = $enrollment->enrollment_status === 'active' ? 'bg-green-100 text-green-700' : 
                          ($enrollment->enrollment_status === 'pending' ? 'bg-orange-100 text-orange-700' : 'bg-red-100 text-red-700');
        @endphp
        
        <div class="tb-course-card mb-4">
            {{-- Header --}}
            <div class="h-20 flex items-center justify-center relative bg-gradient-to-br {{ $isPaid ? 'from-green-500 to-green-700' : 'from-orange-500 to-orange-700' }}">
