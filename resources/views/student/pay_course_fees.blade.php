...
<div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
    ...
    <div class="px-4 py-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white">
        <h1 class="text-xl font-bold">{{ $enrollment->course->title }}</h1>
        <p class="text-green-100 text-sm mt-1">{{ $pendingFees->count() }} pending month(s)</p>
    </div>
    ...
    <div class="p-4">
        ...
        <form method="POST" action="{{ route('student.fees.process-course', $enrollment) }}" enctype="multipart/form-data">
            @csrf
