...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <form method="POST" action="{{ route('tenant.courses.update', $course) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" 
                              class="form-input" placeholder="Course description">{{ old('description', $course->description) }}</textarea>
                </div>

                <!-- Fees Type Selector -->
                @php $currentFeesType = old('fees_type', $course->fees_type ?? 'one_time'); @endphp
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <label class="fees-type-option cursor-pointer">
                            <input type="radio" name="fees_type" value="one_time" class="sr-only fees-type-radio"
                                   {{ $currentFeesType == 'one_time' ? 'checked' : '' }}>
                            <div class="fees-type-card border-2 rounded-xl p-4 text-center transition-all duration-200
                                        {{ $currentFeesType == 'one_time' ? 'border-violet-500 bg-violet-50' : 'border-gray-200 bg-white hover:border-gray-300' }}">
                                <div class="w-10 h-10 rounded-full mx-auto mb-2 flex items-center justify-center
                                            {{ $currentFeesType == 'one_time' ? 'bg-violet-100' : 'bg-gray-100' }}">
                                    <svg class="w-5 h-5 {{ $currentFeesType == 'one_time' ? 'text-violet-600' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"></path>
                                    </svg>
                                </div>
                                <p class="text-sm font-bold {{ $currentFeesType == 'one_time' ? 'text-violet-700' : 'text-gray-600' }}">One Time</p>
                                <p class="text-xs mt-1 {{ $currentFeesType == 'one_time' ? 'text-violet-500' : 'text-gray-400' }}">Single payment to enroll</p>
                            </div>
                        </label>
                        <label class="fees-type-option cursor-pointer">
                            <input type="radio" name="fees_type" value="monthly" class="sr-only fees-type-radio"
                                   {{ $currentFeesType == 'monthly' ? 'checked' : '' }}>
                            <div class="fees-type-card border-2 rounded-xl p-4 text-center transition-all duration-200
                                        {{ $currentFeesType == 'monthly' ? 'border-blue-500 bg-blue-50' : 'border-gray-200 bg-white hover:border-gray-300' }}">
                                <div class="w-10 h-10 rounded-full mx-auto mb-2 flex items-center justify-center
                                            {{ $currentFeesType == 'monthly' ? 'bg-blue-100' : 'bg-gray-100' }}">
                                    <svg class="w-5 h-5 {{ $currentFeesType == 'monthly' ? 'text-blue-600' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                    </svg>
                                </div>
                                <p class="text-sm font-bold {{ $currentFeesType == 'monthly' ? 'text-blue-700' : 'text-gray-600' }}">Monthly</p>
                                <p class="text-xs mt-1 {{ $currentFeesType == 'monthly' ? 'text-blue-500' : 'text-gray-400' }}">Recurring monthly fee</p>
                            </div>
                        </label>
                    </div>
                </div>
                ...
                <!-- Fees Amount -->
                <div id="fees-amount-box" class="rounded-xl p-4 border {{ $currentFeesType == 'monthly' ? 'bg-blue-50 border-blue-200' : 'bg-violet-50 border-violet-200' }}">
                    <div class="flex items-center gap-2 mb-3">
                        <span id="fees-badge" class="text-xs font-bold px-2 py-1 rounded-full {{ $currentFeesType == 'monthly' ? 'bg-blue-100 text-blue-700' : 'bg-violet-100 text-violet-700' }}">
                            {{ $currentFeesType == 'monthly' ? '📅 Monthly Fee' : '💳 One-Time Fee' }}
                        </span>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label for="fees" class="form-label">{{ $currentFeesType == 'monthly' ? 'Monthly Fee (₹) *' : 'Amount (₹) *' }}</label>
                            <div class="relative">
                                <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-500 font-semibold">₹</span>
                                <input type="number" id="fees" name="fees" value="{{ old('fees', $course->fees) }}"
                                       class="form-input pl-7" placeholder="0" min="0" step="0.01" required>
                            </div>
                            <p id="fees-help" class="text-xs mt-1 {{ $currentFeesType == 'monthly' ? 'text-blue-500' : 'text-violet-500' }}">
                                {{ $currentFeesType == 'monthly' ? 'Charged every month' : 'Charged once at enrollment' }}
                            </p>
                        </div>
                        <div>
                    ...
                    <div id="past-month-fee-box" class="mt-3 pt-3 border-t border-blue-200" style="{{ $currentFeesType == 'monthly' ? '' : 'display:none' }}">
                        <label for="past_month_fee" class="form-label">Past Month Access Fee (₹)</label>
                        <div class="relative">
                            <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-500 font-semibold">₹</span>
                            <input type="number" id="past_month_fee" name="past_month_fee" value="{{ old('past_month_fee', $course->past_month_fee) }}"
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    // Run on load if both values are set
                    if (startInput.value && endInput.value) calcDuration();
                    ...
                    radios.forEach(function(radio) {
                        radio.addEventListener('change', function() {
                            ...
                            document.querySelectorAll('.fees-type-option').forEach(function(opt) {
                                ...
                                const title = opt.querySelectorAll('p')[0];
                                const sub = opt.querySelectorAll('p')[1];
                                ...
                                title.className = 'text-sm font-bold ' + (selected ? `text-${color}-700` : 'text-gray-600');
                ...
                <div>
                    ...
                    @if($course->thumbnail)
                        <div class="mb-2">
                            <img src="{{ $course->thumbnail_url }}" alt="Thumbnail" class="w-32 h-24 object-cover rounded-lg">
                        </div>
                    @endif
                        ...
                        @forelse($teachers as $teacher)
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="teachers[]" value="{{ $teacher->id }}" 
                                       ...
                                       {{ in_array($teacher->id, old('teachers', $course->teachers->pluck('id')->toArray())) ? 'checked' : '' }}>
