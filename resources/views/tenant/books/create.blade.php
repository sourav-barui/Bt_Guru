...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <form method="POST" action="{{ route('tenant.books.store') }}" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="type" name="type" class="form-input @error('type') border-red-500 @enderror" required onchange="toggleTypeFields()">
                        <option value="pdf" {{ old('type') === 'pdf' ? 'selected' : '' }}>PDF Only</option>
                        <option value="physical" {{ old('type') === 'physical' ? 'selected' : '' }}>Physical Only</option>
                        <option value="both" {{ old('type') === 'both' ? 'selected' : '' }}>PDF & Physical</option>
                    </select>
                    @error('type')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <!-- PDF Price -->
                <div id="pdf_price_container" class="{{ old('type') === 'physical' ? 'hidden' : '' }}">
                    <label for="pdf_price" class="form-label">PDF Price (₹) <span class="text-red-500 pdf-required">*</span></label>
                    <input type="number" id="pdf_price" name="pdf_price" value="{{ old('pdf_price', 0) }}" step="0.01" min="0"
                ...
                <!-- Physical Price -->
                <div id="physical_price_container" class="{{ old('type') === 'pdf' ? 'hidden' : '' }}">
                    <label for="physical_price" class="form-label">Physical Price (₹) <span class="text-red-500 physical-required">*</span></label>
                    <input type="number" id="physical_price" name="physical_price" value="{{ old('physical_price', 0) }}" step="0.01" min="0"
                ...
                <!-- Stock Quantity -->
                <div id="stock_container" class="{{ old('type') === 'pdf' ? 'hidden' : '' }}">
                ...
                <div class="md:col-span-2">
                    ...
                    <p class="text-sm text-gray-500 mt-1">Recommended: 400x600 pixels, JPG or PNG (max 2MB)</p>
                    @error('cover_image')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <!-- PDF File -->
                <div id="pdf_file_container" class="md:col-span-2 {{ old('type') === 'physical' ? 'hidden' : '' }}">
                    <label for="pdf_file" class="form-label">PDF File <span class="text-red-500 pdf-required">*</span></label>
