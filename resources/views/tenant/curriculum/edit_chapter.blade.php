...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <form method="POST" action="{{ route('tenant.curriculum.chapters.update', [$course, $chapter]) }}">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title', $chapter->title) }}" 
                           class="form-input" placeholder="e.g., Introduction to Algebra" required>
                </div>

                <div>
                    <label for="description" class="form-label">Description</label>
                    <textarea id="description" name="description" rows="3" 
                              class="form-input" placeholder="Brief description of this chapter">{{ old('description', $chapter->description) }}</textarea>
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="order" class="form-label">Display Order</label>
                        <input type="number" id="order" name="order" value="{{ old('order', $chapter->order) }}" 
